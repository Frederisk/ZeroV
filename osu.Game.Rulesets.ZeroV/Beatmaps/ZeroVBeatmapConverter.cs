...
public class ZeroVBeatmapConverter : BeatmapConverter<ZeroVHitObject> {
    ...
    public ZeroVBeatmapConverter(IBeatmap beatmap, Ruleset ruleset)
        ...
        if (beatmap.HitObjects.Any()) {
            this.minPosition = beatmap.HitObjects.Min(getUsablePosition);
            this.maxPosition = beatmap.HitObjects.Max(getUsablePosition);
        }
    }
    ...
    private Int32 getLane(HitObject hitObject) => (Int32)MathHelper.Clamp(
        (this.getUsablePosition(hitObject) - this.minPosition) / (this.maxPosition - minPosition) * ZeroVPlayfield.LANE_COUNT, 0, ZeroVPlayfield.LANE_COUNT - 1);
