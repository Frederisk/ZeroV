...
void main(void)
{
    ...
    vec2 uvCentered = uv + vec2(-0.5, 0.0); // vec2(uv.x - 0.5, uv.y + 1.0);
    // uvCentered.x -= 0.5;

    float distanceToBottom = uvCentered.y + uEffectForce * uvCentered.x * uvCentered.x;

    /*
    float newOne = distanceToBottom;
    if (uvCentered.x > 0.5)
        newOne = uvCentered.y + force * 0.25 - 1.0 * (uvCentered.x - 0.5) * (uvCentered.x - 0.5);
    else if (uvCentered.x < -0.5)
        newOne = uvCentered.y + force * 0.25 - 1.0 * (uvCentered.x + 0.5) * (uvCentered.x + 0.5);
    */
